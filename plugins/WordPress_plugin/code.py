...
class WordPress(abstractPlugin.pluginBlueprint):

    # variable to store the url where the releases will be displayed
    url_check_release = "https://wordpress.org/download/releases/"

    # variable to store the basic url(adding version required in place of *) for downloading
    url_download = "https://wordpress.org/wordpress-*.tar.gz"
    ...
    def check_which_released(self):
        ...
        # finding the latest releases table and storing as rows
        version_rows = parse_tree.find(class_='releases latest').find_all('tr')
        
        # rows contain version, release date
        for i in range(len(version_rows)):
            data = version_rows[i].find_all('td')
    ...
    def update_json(self):
        ...
        for i in range(len(new_releases)):
            ...
            with open(cur_path + "/data/WordPress.json", 'r+') as file:
