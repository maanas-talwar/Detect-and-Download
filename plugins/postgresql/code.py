...
class postgresql(abstractPlugin.pluginBlueprint):
	...
	def check_which_released(self):
		# to detect the name of latest released versions and dump data in json
		...
		for i in range(len(versions_ul)):
			cur_version = str(versions_ul[i].find("strong"))					# returns the text with the strong tag
			cur_version = cur_version[8:]						# removed opening strong tag
			cur_version = cur_version.split("<", 1)[0]			# removed closing strong tag
			# cur_version is now only the text of version

			if(cur_version != ""):
				# since "" is returned for \n(removed strong therfore empty string)
				major_version = str(cur_version).split('.', 1)[0]
				print(major_version)
				released_versions['Major Versions'] = major_version + '.X'
				minor_version = cur_version
				released_versions[major_version + '.X']['Minor Version'] = minor_version

		print(released_versions)
		# store data in json file
		# with open("postgresql.json", "w") as file:
		# 	json.dumps(released_versions, file)
...
# 	url_download = 	url_download = "https://ftp.postgresql.org/pub/source/v*/postgresql-*.tar.gz"
#
# 	# making a bs4 object to parse to the latest release versions
